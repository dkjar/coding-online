...
public class BeanUtils extends org.apache.commons.beanutils.BeanUtils {
	...
	/**
	 * 对象拷贝 数据对象空值不拷贝到目标对象，将databean数据拷贝到tobean
	 * 
	 * @param databean
	 * @param tobean
	 * @param notCopyProperties
	 *            不拷贝的属性
	 * @throws Exception
	 */
	public static void copyBeanNotNull2Bean(Object databean, Object tobean,
			String... notCopyProperties) throws Exception {
		...
		for (int i = 0; i < origDescriptors.length; i++) {
			...
			notCopy.add("class");
			notCopy.add("createDate");
			notCopy.add("creatorId");
			notCopy.add("creatorName");
			notCopy.add("updateDate");
			notCopy.add("updateId");
			notCopy.add("updateName");
			notCopy.add("creatorDepartmentId");
			notCopy.add("creatorDepartmentName");

			if (notCopyProperties != null && notCopyProperties.length > 0) {
				notCopy.addAll(Arrays.asList(notCopyProperties));
			}
			if (notCopy.contains(name)) {
				continue;
			}
