...
<table	<#if treeField??>class="treegrid" data-treefield="${treeField}"<#else>class="datagrid"</#if> <#if singleselect??&&singleselect==0>data-singleselect="false"</#if>
	data-url="${r'$'}{ProjectPath}/${entityName?uncap_first}Controller/select${table.entityname?uncap_first}data.htm?${fcolumn.columncode}=${r'$'}{${fcolumn.columncode}}"  
...
<div id="${entityName?uncap_first}-toolbar" >
	<div class="toolbars">
		<div style="float:left;">
			...
			<b action="method:'ajax',grid:'id',url:'../${entityName?uncap_first}Controller/add${entityName?uncap_first}${table.entityname?uncap_first}.htm?${fcolumn.columncode}=${r'$'}{${fcolumn.columncode}}'"  class="select-button hidden">添加</b>
		</div>
	</div>
</div>
