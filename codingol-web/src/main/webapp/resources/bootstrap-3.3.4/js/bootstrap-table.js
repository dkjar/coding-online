...
!function ($) {
    ...
    var translateLink = function (column, items) {
    	var link  = column.link;
    	if(link){
    		var fields = column.link.match(/\{\w+\}/g);
    ...
    BootstrapTable.LOCALES['zh-CN'] = {
        ...
        formatShowingRows: function (pageFrom, pageTo, totalRows) {
            return sprintf('第%s页 %s-%s记录', pageFrom, pageTo, totalRows);
        },
    ...
    BootstrapTable.prototype.initContainer = function () {
        this.$container = $([
            ...
            '</div>',
            '</div><div class="clearfix"></div>'].join(''));
    ...
    BootstrapTable.prototype.initHeader = function () {
        ...
        $.each(this.options.columns, function (i, column) {
            ...
            that.header.links.push(column.link);
    ...
    BootstrapTable.prototype.initBody = function (fixedScroll) {
        ...
        for (var i = this.pageFrom - 1; i < this.pageTo; i++) {
            ...
            $.each(this.header.fields, function (j, field) {
                ...
                var link = translateLink(column, item);
                ...
                if (column.checkbox || column.radio) {
                    ...
                    text = that.options.cardView ?
                        ...
                        [sprintf('<td%s %s %s %s>', id_, class_, style, data_),
                         link? sprintf('<a href="%s">%s</a>', link, value):value,
                            '</td>'].join('');
        ...
        this.$body.find('> tr > td').off('click').on('click', function () {
            ...
            if (that.options.clickToSelect) {
                if (that.header.clickToSelects[$tr.children().index($(this))]) {
                	var c  = $tr.find(sprintf('[name="%s"]', that.options.selectItemName))[0];
                	if(c)
                		c.click();
                }
            }
    ...
    BootstrapTable.prototype.fitHeader = function () {
        ...
        $fixedHeader.css({
            'margin-right': scrollWidth
        }).find('table').html('').attr('class', that.$el.attr('class')).append(that.$header_);
