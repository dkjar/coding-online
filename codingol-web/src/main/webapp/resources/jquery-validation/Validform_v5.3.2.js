...
(function($, win, undef) {
	...
	var Validform = function(forms, settings, inited) {
		...
		forms.each(function() {
			...
			// 防止表单按钮双击提交两次;
			curform.validform_status = "normal"; // normal | posting |
													// posted;
	...
	Validform.util = {
		...
		getNullmsg : function(curform) {
			...
			label = obj.siblings(label).eq(0).text()
					...
					|| obj.parent().prev(label).eq(0).text()
					|| obj.parent().prev().find(label).eq(0).text();
		...
		_regcheck : function(datatype, gets, obj, curform) {
			var curform = curform, info = null, passed = false, reg = /\/.+\//g, regex = /^(.+?)(\d+)-(\d+)$/, type = 3;// default
																														// set
																														// to
																														// wrong
																														// type,
		...
		check : function(curform, subpost, bool) {
			...
			if(!ajaxurl)
				...
				if (inputval == this.validform_lastval && this.validform_lasturl == inputurl && !$(this).attr("recheck")
						&& subpost == "") {
					return flag.passed ? true : false;
				}
			...
			if(ajaxurl)
				this.validform_lasturl = ajaxurl;
		...
		resetForm : function() {
			...
			brothers.find("[datatype]").removeData("cked").removeData(
				...
				this.validform_lasturl = null;
				this.isFirestVisible = false;
			});
	...
	Validform.prototype = {
		...
		config : function(setup) {
			/*
			 * config={ url:"ajaxpost.php",//指定了url后，数据会提交到这个地址; ajaxurl:{
			 * timeout:1000, ... }, ajaxpost:{ timeout:1000, ... } }
			 */ 
			var obj = this;
			setup = setup || {};
			$(obj.forms).each(
...
$(function(){
	$(":input").each(function(){ 
		...
		if(datatype){
			...
			if(!ignore){
				if($(this).hasClass("select-text")){
					$(this).parent(".select-table").addClass("left");
					$(this).parent(".select-table").after("<span class='red'>*</span>");
				}else{
