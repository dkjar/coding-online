// change rows to limit ; _parentId to pid; treeField to treefield ; 
// singleSelect to singleselect; form load function add line " form.find("p[name=\""+name+"\"]").text(val); "
// tree node field text  is changed to name;
/**
 * jQuery EasyUI 1.4.2
 * 
...
$.ajax({url:data,cache:false,data:_4dd,dataType:"json",success:function(data){
_4de(data);
},error:function(){
...
form.find("p[name=\""+name+"\"]").text(val);
}
}
}
...
if(opts.pagination){
$.extend(_84a,{page:opts.pageNumber,limit:opts.pageSize});
}
if(opts.sortName){
     ...
     * 级联选择 
     ...
     cascadeCheck : function(target,param){
         ...
         function selectParent(target,id,idField,status){  
             ...
             if(parent){
            	 var children = $(target).treegrid('getChildren',parent.id);
            	 var selected = $(target).treegrid('getSelections');
            	 ...
            	 if(!isallselect){
            		 $(target).treegrid('unselect',parent.id);
            		 return;
            	 }
            	 
                  if(status)  
                     $(target).treegrid('select',parent.id);  
                 else  
                     $(target).treegrid('unselect',parent.id);
                  
                 
                 selectParent(target,parent.id,idField,status);  
             }  
         }  
         /** 
          * 级联选择子节点 
